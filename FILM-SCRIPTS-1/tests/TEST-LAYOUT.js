...
let TESTREC = {type:RECT,name:"bezrect0",x:170,y:140,width:50,height:50,rotation:45,strokeColor:orange,radius:4,fillColor:green,}
addBlock(TESTREC)
...
for (x = 0; x < layoutx ; x++)
{
	for (y = 0; y < layouty ; y++)
	{
		let name = "bezcir".concat(x.toString()).concat(y.toString())
		let circle =
		{
			type:CIRCLE,
			name:name,
			...
			radius:40,
			fillColor:green,
			animator0:{value:"x", amount:2, min:0, max:10, type:BOUNCE},
			childBlock0: {type:CIRCLE, name:"bezcircl0s", x:0, y:0, strokeColor:red, radius:10, fillColor:blue},
		}
	addBlock(circle)
	}
}
